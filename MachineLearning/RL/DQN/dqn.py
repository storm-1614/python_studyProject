...
class ReplayBuffer:
    ...
    def add(self, state, acton, reward, next_state, done):  # 将数据加入 buffer
        self.buffer.append((state, acton, reward, next_state, done))

    def sample(self, batch_size):
        """从 buffer 中采样数据，数量为 batch_size"""
        transitions = random.sample(
            self.buffer, batch_size
        )  # 从 buffer 中随机抽取 batch_size 条经验记录
        state, action, reward, next_state, done = zip(*transitions)  # 打包参数
