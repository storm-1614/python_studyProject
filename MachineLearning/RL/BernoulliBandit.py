...
class BernoulliBandit:
    ...
    def __init__(self, K: int) -> None:
        self.probs = np.random.uniform(
            size=K
        )  # 随机生成 K 个 0~1 的数，作为拉动每根拉杆的获奖概率
        self.best_idx = np.argmax(self.probs)  # 获奖概率最大的拉杆
        self.best_prob = self.probs[self.best_idx]  # 最大获奖概率
        self.K = K

    def step(self, k: int | np.intp):
        """当玩家选择了k号拉杆后，根据拉动该老虎机的k号拉杆活动奖励的概率返回1(获奖)或0（未获奖)"""
...
class EpsilonGreedy(Solver):
    ...
    def run_one_step(self):
        if np.random.random() < self.epsilon:
            ...
            k = int(np.argmax(self.estimates))  # 选择期望奖励估值最大的拉杆
...
class DecayingEpsilonGreedy(Solver):
    """
    epsilon 值随时间衰减的 epsilons-贪婪算法，继承 Solver 类
    """
...
class UCB(Solver):
    ...
    def run_one_step(self):
        ...
        k = int(np.argmax(ucb))  # 选出上置信界最大的拉杆
        r = self.bandit.step(k)
        self.estimates[k] += 1.0 / (self.counts[k] + 1) * (r - self.estimates[k])
        return k
