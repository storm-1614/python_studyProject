...
class Sarsa:
    ...
    def take_acion(self, state):  # 选取下一步的操作，具体实现为 epsilon-贪婪
    ...
    def best_action(self, state):  # 用于打印策略
        ...
        for i in range(self.n_action):  # 若两个动作都价值一样，都会记录下来
...
class nstep_Sarsa:
    """ n 步 Sarsa 算法 """
    def __init__(self, n, ncol, nrow, epsilon, alpha, gamma, n_action = 4) -> None:
    ...
    def update(self, s0, a0, r, s1, a1, done):
        ...
        if len(self.state_list) == self.n: # 若保存的数据可以进行 n 步更新
            ...
            # n 步 Sarsa 的主要更新步骤
            self.Q_table[s, a] += self.alpha * (G - self.Q_table[s, a])
        if done: # 如果到达终止状态，即将开始下一条序列，则将列表其全清空
            self.state_list = []
            self.action_list = []
            self.reward_list = []
